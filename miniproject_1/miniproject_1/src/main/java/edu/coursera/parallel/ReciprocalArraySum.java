...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            // TODO
            // Compute sum of reciprocals of array elements
//            if(endIndexExclusive - startIndexInclusive <= 5000){
...
//            }
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for(i=0; i<taskNum-1; i++){
            int end = getChunkEndExclusive(i, taskNum, input.length);
            tasks[i] = new ReciprocalArraySumTask(start, end, input);
            tasks[i].fork();
            start = end;
        }
        tasks[i] = new ReciprocalArraySumTask(start, getChunkEndExclusive(i, taskNum, input.length), input);
        tasks[i].compute();

        double sum = tasks[i].getValue();
